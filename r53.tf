...
resource "aws_route53_record" "ses_verification_record" {
  ...
  type    = "TXT"
  ttl     = 300
  records = [aws_ses_domain_identity.landhoderslaw_domain.verification_token]
}
...
resource "aws_route53_record" "ses_dkim_record" {
  count   = 3
  zone_id = data.aws_route53_zone.selected.zone_id
  name    = "${element(aws_ses_domain_dkim.landhoderslaw_domain_dkim.dkim_tokens, count.index)}._domainkey.${var.ses_domain_name}"
  type    = "CNAME"
  ttl     = 300
  records = ["${element(aws_ses_domain_dkim.landhoderslaw_domain_dkim.dkim_tokens, count.index)}.dkim.amazonses.com"]
}
